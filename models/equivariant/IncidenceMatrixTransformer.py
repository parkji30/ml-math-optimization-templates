...
class MLP(eqx.Module):
	...
	def __init__(self, dim: int, hidden_dim: int, *, key: jax.random.PRNGKey, dtype: jnp.dtype = jnp.float32):
...
class TransformerAttentionBlock1D(eqx.Module):
	...
	def __init__(
		...
		*,
		...
		self.attention = IncidenceAttentionHead(embed_dim, attention_hidden_dim, key=key1, dtype=dtype)
		self.mlp = MLP(embed_dim, mlp_hidden_dim, key=key2, dtype=dtype)

	def __call__(self, x: jax.Array, mask: jax.Array, attention_along_channel: bool) -> jax.Array:
		# Attention block
...
class ConstantChannelTransformerCLS(eqx.Module):
	...
	def __init__(
		...
		*,
		...
		self.blowup_layer = [eqx.nn.Linear(3, attention_hidden_dim, key=key, dtype=dtype), jax.nn.gelu]

		self.encoder_blocks = [
			TransformerAttentionBlock1D(
				attention_hidden_dim, attention_hidden_dim, attention_hidden_dim, key=key, dtype=dtype
			)
		...
		self.ff_layer = [
			eqx.nn.Linear(2 * attention_hidden_dim, mlp_hidden_dim, key=key, dtype=dtype),
			jax.nn.gelu,
